...
public class EditorRecordAndSetTransforms {
    ...
    private static void setStoredChildLocator(GameObject selected) {
        ...
        ChildLocator newChildLocator = selected.GetComponent<ChildLocator>();
        if(newChildLocator == null) {
            newChildLocator = selected.AddComponent<ChildLocator>();
        }

        List<Transform> storedTransforms = selected.GetComponentsInChildren<Transform>().ToList();

        newChildLocator.TransformPairs = new ChildLocator.NameTransformPair[storedChildLocator.TransformPairs.Length];

        for (int i = 0; i < newChildLocator.TransformPairs.Length; i++) {

            ChildLocator.NameTransformPair newPair = newChildLocator.TransformPairs[i];
            ...
            newPair.transform = storedTransforms.Find(tran => { 
                return tran.name == storedPair.transform.name; 
            });

            if(newPair.transform == null && storedPair.transform != null) {

                string log = $"Could not find transform {storedPair.name}. Duplicating object from old pair";
                newPair.transform = Object.Instantiate(storedPair.transform);

                newPair.transform.parent = storedTransforms.Find(tran => {
                    return tran.name == storedPair.transform.parent.name;
                });

                if(newPair.transform.parent == null) {
                    log += $"\n could not find old transform's parent {storedPair.transform.parent.name}";
                }

                Debug.Log(log);
            }

            newChildLocator.TransformPairs[i] = newPair;
        }
    }
